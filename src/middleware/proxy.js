          ...
          if ('cx-layout' in response.headers) {
            ...
            var extract = function extract(data) {
              ...
              extractSlots(data, function (err, slots) {
                ...
                var newLayoutUrl = new URL(layoutUrl);
                var globalQueryParamsToAllowForLayout = config.passThroughQueryParamsToLayout || [];
                var backendQueryParamsToAllowForLayout = backend.passThroughQueryParamsToLayout || [];
                var allQueryParamsToAllowForLayout = globalQueryParamsToAllowForLayout.concat(backendQueryParamsToAllowForLayout);

                allQueryParamsToAllowForLayout.forEach(function (allowedParam) {
                  if (req.query.hasOwnProperty(allowedParam)) {
                    newLayoutUrl.searchParams.set(allowedParam, req.query[allowedParam]);
                  }
                });

                var toggle = newLayoutUrl.searchParams.has('toggle') ? newLayoutUrl.searchParams.get('toggle').split(',') : [];
                if (backend.hasOwnProperty('toggle')) {
                  toggle = toggle.concat(backend.toggle);
                }
                if (toggle.length > 0) {
                  newLayoutUrl.searchParams.set('toggle', toggle.join(','));
                }
                layoutUrl = newLayoutUrl.toString();
                ...
                var layoutOptions = {
                  ...
                  headers: {
                    ...
                    'x-target-application': req.backend.name,
