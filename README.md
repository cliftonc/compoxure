...
### Pass parameters down to the service layout template
In order to pass some query string parameters from the top level request down to the service layout template you need to whitelist them in the configuration file. You can do the whitelisting on the global level and extend it on per backend basis. 

```
passThroughQueryParamsToLayout: ['queryParamName', ....]
```

## Toggle layout features from the upper layer config level
In order to add feature toggles flag to the layout call just add the toggle variable to your backend config. The config overrides the target application configuration.
```
{
  toggle: ['onetrust', ....]
}
